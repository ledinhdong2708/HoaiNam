...
class XinNghiPhepService {
  ...
  static Future<dynamic> FetchXinNghiPhepByDate(
    ...
    final url =
        '${SERVER_IP}${apiXinNghiPhep}/date?UserID=${body['userID']}&day=${day.toString()}&month=${month.toString()}&year=${year.toString()}';
    final uri = Uri.parse(url);
  ...
  static Future<bool> submitData(Map body) async {
    // final SharedPreferences prefs = await _prefs;
    const url = '${SERVER_IP}${apiXinNghiPhep}';
    final uri = Uri.parse(url);
    // String? value = await prefs.getString('jwt');
    String? value = await storage.read(key: 'jwt');
  ...
  static Future<bool> submitDataPh(body) async {
    // final SharedPreferences prefs = await _prefs;
    const url = '${SERVER_IP}${apiXinNghiPhep}/ph';
    final uri = Uri.parse(url);
    // String? value = await prefs.getString('jwt');
    String? value = await storage.read(key: 'jwt');
    final token = json.decode(value!);
  ...
  static Future<bool> updateDataPh(int id, Map body) async {
    print("body: ");
    print(body);
    String? value = await storage.read(key: 'jwt');
    // String? value = await prefs.getString('jwt');
    ...
    body["appID"] = token['appID'];
    final url = '${SERVER_IP}${apiXinNghiPhep}/ph$id';
    final uri = Uri.parse(url);
    // ${body['userID']
    final response =
        await http.put(uri, body: jsonEncode(body), headers: headers);
  ...
  static Future<dynamic> PH_FetchHocPhiByStudent() async {
    try {
      ...
      final url = '${SERVER_IP}${apiXinNghiPhep}/ph?id=${body['studentID']}';
      final uri = Uri.parse(url);
      final response = await http.get(uri, headers: headers);
