...
class ChamCongService {
  static Future<List?> FetchClassNameTeacher(day, month, year) async {
    // final SharedPreferences prefs = await _prefs;
    final url =
        '${SERVER_IP}${apiDiemDanhGiaoVien}/getbygiaovien?day=$day&month=$month&year=$year';
  ...
  static Future<List?> FetchDiemDanhStatusUpdate(day, month, year) async {
    // final SharedPreferences prefs = await _prefs;
    final url =
        '${SERVER_IP}${apiDiemDanhGiaoVien}/statusupdateGV?day=$day&month=$month&year=$year';
    final uri = Uri.parse(url);
    String? value = await storage.read(key: 'jwt');
    // String? value = await prefs.getString('jwt');
    final body = json.decode(value!);
  ...
  static Future<bool> submitData(List<Map> body) async {
    // final SharedPreferences prefs = await _prefs;
    const url = '${SERVER_IP}${apiDiemDanhGiaoVien}';
    final uri = Uri.parse(url);
    String? value = await storage.read(key: 'jwt');
    // String? value = await prefs.getString('jwt');
  ...
  static Future<bool> updateDataDiemDanh(Map body) async {
    // final SharedPreferences prefs = await _prefs;
    const url = '${SERVER_IP}${apiDiemDanhGiaoVien}/updateDiemDanh';
    final uri = Uri.parse(url);
    String? value = await storage.read(key: 'jwt');
    if (value == null) {
  ...
  static Future<bool> updateData(List<Map> body) async {
    ...
    final url = '${SERVER_IP}${apiDiemDanhGiaoVien}/updatemultipleGV';
    final uri = Uri.parse(url);
