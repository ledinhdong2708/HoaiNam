...
class HocSinhService {
  ...
  static Future<List?> FetchByKhoaHocAndClassDate(
      idkhoahoc, idclass, days, months, years) async {
    // final SharedPreferences prefs = await _prefs;
    final url =
        '${SERVER_IP}${apiStudents}/bykhoahocclassdate?khoaHocId=$idkhoahoc&classId=$idclass&day=$days&month=$months&year=$years';
    final uri = Uri.parse(url);
    String? value = await storage.read(key: 'jwt');
    // String? value = await prefs.getString('jwt');
    final body = json.decode(value!);
    ...
    if (response.statusCode == 200) {
      final json = jsonDecode(response.body);
      final result = json["studentDiemDanhs"] as List;
      return result;
    } else {
      return null;
    }
