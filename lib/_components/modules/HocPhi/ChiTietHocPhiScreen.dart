...
class ChiTietHocPhiScreen extends StatefulWidget {
  final Map? item;
  final int studentID;
  const ChiTietHocPhiScreen({Key? key, this.item, required this.studentID})
      : super(key: key);
...
class _ChiTietHocPhiScreenState extends State<ChiTietHocPhiScreen> {
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
                      ...
                      children: [
                        ...
                        Expanded(
                          ...
                          child: Align(
                            ...
                            child: Text(_formatter.formatDouble(total)),
                            // child: Text(_formatter.format(total.toString())),
                          ),
                        ),
                      ],
                    ),
            ...
            SizedBox(height: 15),
            ElevatedButton(
              onPressed: submitData, //data.length > 0 ? updateData : null,
              child: Padding(
                padding: const EdgeInsets.all(10.0),
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.center,
  ...
  Future<void> FetchDataByMaterHocPhi() async {
    final response = await HocPhiModelService.FetchDataByMaterHocPhi();
    if (response != null) {
      setState(() {
        FillDataListStatusTrue(response);
      });
    } else {
  ...
  FillDataListStatusTrue(List dataFunc) {
    for (int i = 0; i < dataFunc.length; i++) {
      if (dataFunc[i]["isCompleted"] == true) {
        dataFunc[i]["status"] = true;
        dataFunc[i]["total"] = 0;
        dataFunc[i]["quantity"] = 1;
        data.add(dataFunc[i]);
      }
    }
  }
  ...
  Future<void> navigateToAddPage() async {
    ...
    // var expectedList = data.toSet().intersection(result.toSet()).toList();
  }

  // double totalList(List dataTotal) {
  //   double _total = 0;
  //   // dataTotal.forEach((element) {
  //   //   _total += double.parse(element["total"]);
  //   // });
  //   for (int i = 0; i < dataTotal.length; i++) {
  //     _total += double.parse(dataTotal[i]["total"]);
  //   }
  //   return _total;
  // }
  ...
  ChangeValueQuantityInList(value, index) {
    for (int i = 0; i < data.length; i++) {
      if (index == i) {
        data[i]["quantity"] = value.toString().replaceAll(',', '');
      }
    }
  }
  ...
  Future<void> submitData() async {
    print(body);
    final isSuccess = await HocPhiModelService.submitData(body);
    ...
    if (isSuccess) {
      ...
      var InsertbodyFirebase = {
        ...
        "priority": "high",
        "notification": {
          "title": "Thêm mới học phí",
          "body": "Học phí ${monthEditingController.text} đã được tạo mới !"
        }
      };
  ...
  Map get body {
    ...
    //     "userId": 0,
    //     "studentId": 0,
    //     "createDate": "2023-12-25T07:40:28.616Z",
    //     "updateDate": "2023-12-25T07:40:28.616Z",
    //     "content": "string",
    //     "appID": 0,
    //     "materHocPhiId": 0,
    //     "total": 0,
